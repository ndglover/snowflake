...
# C# Snowflake Driver for Apache Arrow ADBC
...
- **Assembly / package:** `AdbcDrivers.Snowflake`
...
```bash
dotnet test csharp/test/AdbcDrivers.Snowflake.Tests --filter "Category=Unit"
SNOWFLAKE_TEST_CONFIG_FILE=/path/to/config.json dotnet test csharp/test/AdbcDrivers.Snowflake.Tests --filter "Category=Integration"
```

Per-file breakdown: [test suite readme](../../test/AdbcDrivers.Snowflake.Tests/readme.md). Session-lifecycle
...
Representative results (multi-run means — C# n=5, interop n=10 — 2026-07-14, Release/net8.0):

| Rows fetched | C# | Interop (Go) | C# / Interop |
|---|---|---|---|
| 100 | 143 ms | 116 ms | 1.23× |
| 1,000 | 244 ms | 306 ms | 0.80× |
| 1,000,000 | 2,813 ms | 3,699 ms | **0.76×** |

At scale the C# driver runs at parity or ahead of the Go driver (bounded parallel chunk
...
comparisons use interleaved C#/interop runs and multi-run means, never cross-session
absolutes.
